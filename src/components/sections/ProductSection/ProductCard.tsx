    ...
    <div ref={contentRef} className="opacity-0">
        ...
        <ul className="space-y-2">
          {features.map((feature, i) => (
            <li key={i} className="flex items-start gap-2">
              <span className="text-foreground text-[1rem] leading-8 tracking-normal font-manrope font-normal">•</span>
              {feature}
            </li>
          ))}
        </ul>
      ...
      <div className="flex flex-wrap gap-8">
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className={`px-8 py-3 rounded-full bg-hero-title text-white transition-all font-semibold text-xl leading-5 tracking-normal font-raleway`}
        >
          Learn More
        </motion.button>
        <motion.button
